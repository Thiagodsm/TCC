...
int main()
{
	...
	char nameTable[100];
    char constraints[100];
    char *aux0;
	aux0 = fgets(nameTable, sizeof(nameTable), arqProblems); //read first line of arqProblems file
	...
	{
		// read the name of instances, the constraints, debug mode, local search module, maximum time, maximum number of runs, maximum number of threads
		fscanf(arqProblems,"%s %s %d %d %d %d %d %d %d", nameTable, constraints, &debug, &ls, &MAXTIME, &MAXRUNS, &MAX_THREADS, &n, &m);
		strcpy(instance,nameTable);
        
		//read the informations of the instance
		ReadData(nameTable, constraints, dist, constraints_matrix, CL, ML);
...
void BRKGA()
{
    ...
    for (int i=0; i<p; i++)
    {
        ...
        ind = Decoder(ind,n,m,dist,constraints_matrix,CL,ML);
    ...
    while(1)
    {
        ...
        for (int i = (int)(p*pe); i < p - (int)(p*pm); i++)
        {
            ...
            PopInter[i] = Decoder(PopInter[i],n,m,dist,constraints_matrix,CL,ML);
            ...
            if (R >= 1 || stagnation > 20) 
            {
                ...
                for (int i=0; i < (int)(p*pe); i++)
                {
                    if (Pop[i].promising == 1){
                        ...
                        // generate caotics individual (half elite, half mutant)
                        double beta = 0.5; 
                        Pop[i] = Perturbation(Pop[i],beta,n);
                        Pop[i] = Decoder(Pop[i],n,m,dist,constraints_matrix,CL,ML);
    ...
    if (debug){
        ...
        printf("\n\npenalidades: %d",bestSolution.penalidades);
    }
...
TSol Perturbation(TSol s, double beta, int n)
{
    for (int k=0; k<n*beta; k++)
    {        
        // generate a new random-key
        int pos1 = irandomico(0, n-1); 
        s.vec[pos1].rk = randomico(0,1);
    }
...
{
    return ((double)(rand()%10000)/10000.0)*(max-min)+min;  //rand()%10000 gera um numero aleatorio entre 0 9999 
